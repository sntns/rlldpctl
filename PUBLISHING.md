...
This document explains how to publish new releases of `rlldpctl` to crates.io using `cargo-release`.
...
1. Go to the [Actions tab](https://github.com/sntns/rlldpctl/actions/workflows/release.yml) in your GitHub repository
...
`rlldpctl` publishes as a single library crate - the `tokio` feature (async
`AsyncClient`/`AsyncSubscription`) ships in the same crate, opt-in, so there's
nothing extra to publish separately for it.

## Installation After Publishing

Users can install the crate as a library dependency:

```toml
[dependencies]
rlldpctl = "0.2.0"
# or, for the async API:
rlldpctl = { version = "0.2.0", features = ["tokio"] }
```
